...
int org_main(int argc, const char *argv[]) {
  ...
  int main(int argc, const char *argv[]) {
	...
	WebPPicture* picture = new WebPPicture[NPicPool];
	...
	std::string* PicPoolList = new std::string[NPicPool];
	std::ifstream PicRead(in_file);
	for(int i=0;i<NPicPool;i++){
	  std::getline(PicRead, PicPoolList[i]);
	};
	PicRead.close();

	for(int i=0;i<NPicPool;i++)
	  std::cout << PicPoolList[i] << std::endl;
	...
	for(int i=0;i<NPicPool;i++){
	  picture[i].use_argb = (config.lossless || config.preprocessing > 0 ||
	...
	for(int i=0;i<NPicPool;i++){
	  ...
	  int ok = ReadPicture(PicPoolList[i].c_str(), &picture[i], keep_alpha,
	  ...
	  std::cout << picture[i].width << "  " << picture[i].height << std::endl;
	...
	if (config.use_ocl) {
	  kernel_status = CreateKernel(); // create_kernel.c
	  if (kernel_status) {
	...
	// If using opencl, set kernel arguments first.
  	if (config.use_ocl) {
  	  kernel_status = CreateDeviceBuffers();
  	  if (kernel_status) {
	...
	//
	printf("INFO: WebPEncodeAsync Starts... \n");
	StartProfiling(&watch); 

	int ok = WebPEncodeAsync(&config, &picture);
	/* if (ok) {  */
	...
	printf("INFO: WebPEncodeAsync Finished. Computation time is %f (ms) \n\n", watch_time);
