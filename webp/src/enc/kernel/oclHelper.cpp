...
static void getDeviceInfo(oclHardware& hardware)
{
  ...
  size_t size = 0;
  cl_int err = clGetDeviceInfo (hardware.mDevice, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(localMemSize), &localMemSize, &size);
  if ( err != CL_SUCCESS) {
	std::cout << oclErrorCode(err) << "\n";
	return;
  }
  ...
  err = clGetDeviceInfo (hardware.mDevice, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(maxDimension), &maxDimension, &size);
  if ( err != CL_SUCCESS) {
	std::cout << oclErrorCode(err) << "\n";
	return;
  }
...
oclHardware getOclHardware(cl_device_type type, char *target_device)
{
  ...
  printf("INFO: Number of Platforms: %d\n", platformCount);
  ...
  for (cl_uint i = 0; i < platformCount; i++) {
    ...
    printf("INFO: Number of devices for platform %d: %d\n", i, deviceCount);
	...
	for (idev=0; idev < deviceCount; idev++) {
	  ...
	  printf("INFO: target_device found:   %s\n", deviceName);
	
	  if (strcmp(deviceName, target_device) == 0) {
		device_id = devices[idev];
		printf("INFO: target_device chosen:  %s\n", deviceName);
		break;
	  }            
	}
	...
	if (idev == deviceCount) {
	  fprintf(stderr, "ERROR: target device %s not found \n: %s %d %s", 
			  target_device,  __func__, __LINE__, oclErrorCode(err));
	  return hardware;
	}
