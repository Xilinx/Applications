...
static HistoWorkerData * HistogramAddThreshAlloc(HistoWorkerData *worker_data, int num_workers) {
  ...
  worker_data = (HistoWorkerData *)WebPSafeMalloc((uint64_t)num_workers,
                                                  sizeof(*worker_data));
  if (worker_data == NULL) {
    return NULL;
  }
...
static void HistogramAddThreshMt(const VP8LHistogramSet* const orig_histo,
                                 ...
                                 uint16_t* const symbols, int number_thread) {
...
int VP8LGetHistoImageSymbols(int xsize, int ysize,
                             ...
                             uint16_t* const histogram_symbols, int number_thread) {
  int ok = 0;
