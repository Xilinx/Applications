...
static int TBufferNewPageFromKernel(VP8TBuffer* const b, VP8TBufferKernel* const tokens_kernel, int tokens_index) {
  ...
  // printf("jj:%d page:%d b:%d b->pages:%d b->pages_->next_:%d b->last_page_:%d *(b->last_page_):%d  b->left_:%d b->tokens_:%d sizeof(page->next_):%d sizeof(unsigned long long):%d\n",
  //     j, page, b, b->pages_, b->pages_->next_, b->last_page_ , *(b->last_page_), b->left_, b->tokens_, sizeof(page->next_), sizeof(unsigned long long));
  // }
  ...
  *b->last_page_ = page;
  ...
  b->tokens_ = (token_t*)TOKEN_DATA(page);
    ...
    // printf("j:%d page:%d b:%d b->pages:%d b->pages_->next_:%d b->last_page_:%d *(b->last_page_):%d  b->left_:%d b->tokens_:%d sizeof(page->next_):%d sizeof(unsigned long long):%d\n\n",
    //     j, page, b, b->pages_, b->pages_->next_, b->last_page_ , *(b->last_page_), b->left_, b->tokens_, sizeof(page->next_), sizeof(unsigned long long));
    // j++;

  memcpy(b->tokens_, tokens_kernel->tokens_ + tokens_index * TOKENS_COUNT_PER_PAGE, TOKENS_COUNT_PER_PAGE * TOKENS_SIZE);
...
static WEBP_INLINE uint32_t AddToken(VP8TBufferKernel* const b,
...
static WEBP_INLINE void AddConstantToken(VP8TBufferKernel* const b,
