...
static WEBP_INLINE void sync_write(IpSync *const ip_sync, int r, int c,
                              const int cols) {
#ifdef WEBP_USE_THREAD
  const int nsync = ip_sync->sync_range;
  int cur;
...
static WEBP_INLINE void ThreadInversePredictorRows(uint32_t* data, const VP8LTransform* const transform,
                                       int y_start, int y_end, IPWorkerData *const ip_data,
...
static int InversePredictorWorker(IpSync *const ip_sync,
                                  IPWorkerData *const ip_data) {
  ThreadInversePredictorRows(ip_data->data, ip_data->transform,
                             ip_data->start, ip_data->stop,ip_data,
                             ip_sync);
  return 1;
}
...
static void InversePredictorAlloc(IpSync *ip_sync, int rows,
                           int width, int num_workers) {
  ip_sync->rows = rows;
#ifdef WEBP_USE_THREAD
  {
    int i;
    ip_sync->mutex_ = (pthread_mutex_t *)WebPSafeMalloc((uint64_t)rows,
                                                        sizeof(*ip_sync->mutex_));
    if (ip_sync->mutex_) {
      for (i = 0; i < rows; ++i) {
    ...
    ip_sync->cond_ = (pthread_cond_t *)WebPSafeMalloc((uint64_t)rows,
                                                      sizeof(*ip_sync->cond_));
    if (ip_sync->cond_) {
      for (i = 0; i < rows; ++i) {
  ...
  ip_sync->ipdata = (IPWorkerData *)WebPSafeMalloc((uint64_t)num_workers,
                                                   sizeof(*ip_sync->ipdata));
  ip_sync->num_workers = num_workers;

  ip_sync->cur_col = (int *)WebPSafeMalloc((uint64_t)rows,
                                          sizeof(*ip_sync->cur_col));
...
// Inverse prediction.
static void PredictorInverseTransformMt(const VP8LTransform* const transform,
                                        int y_start, int y_end, uint32_t* data, int num_workers) {
  IpSync ip_row_sync;
  ...
  if (!ip_sync->sync_range || rows != ip_sync->rows ||
      num_workers > ip_sync->num_workers) {
