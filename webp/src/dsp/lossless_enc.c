...
#include "../enc/kernel/getresidule.h"
...
// Returns best predictor and updates the accumulated histogram.
static int GetBestPredictorForTile2(int width, int height, int group_width, int group_height, int bits,
                                    int tile_x, int tile_y, uint32_t* residual, int accumulated[4][256]) {
  const int kNumPredModes = 14;
  float best_diff = MAX_DIFF_COST;
  int best_mode = 0;
  int mode, i, j, y;
  ...
  for (mode = 0; mode < kNumPredModes; ++mode) {
    ...
    for (y = 0; y < max_y; ++y) {
      ...
      for (x = 0; x < max_x; ++x) {
        ...
        int offset =  ( (global_width / group_width) * group_y + group_x) * group_width * group_height * kNumPredModes;
    ...
    //calculate best mode
    cur_diff = PredictionCostSpatialHistogram(
        (const int (*)[256])accumulated, histo_argb);
    if (cur_diff < best_diff) {
      int (*tmp)[256] = histo_argb;
      histo_argb = best_histo;
      best_histo = tmp;
...
static WEBP_INLINE void VP8LResidualImageRec(int width, int height, int bits, int low_effort,
  ...
  const int residual_size = ((width + group_width -1 ) / group_width) * group_width * ((height + group_height -1 ) / group_height) * group_height * kNumPredModes * sizeof(uint32_t);
  int arg = 0;

  size_t globalSize[] = {((width + group_width -1 ) / group_width) * group_width, ((height + group_height -1 ) / group_height) * group_height};
    ...
    for(group_x = 0; group_x < globalSize[0]/group_width; group_x++) {
      for(local_y = 0; local_y < group_height; local_y++) {
        ...
        for(local_x = 0; local_x < group_width; local_x++) {
          ...
          int residule_offset = (group_y * globalSize[0] / group_width + group_x) * group_width * group_height * kNumPredModes + local_y * group_width * kNumPredModes + local_x * kNumPredModes;//for save res
  ...
  for (tile_y = 0; tile_y < tiles_per_col; ++tile_y) {
    ...
    for (tile_x = 0; tile_x < tiles_per_row; ++tile_x) {
      const int pred = GetBestPredictorForTile2(width, height, group_width, group_height, bits, tile_x, tile_y, residual, histo);
...
static WEBP_INLINE void VP8LResidualImageOcl(int width, int height, int bits, int low_effort,
                                             uint32_t* const argb, uint32_t* const argb_scratch,
                                             uint32_t* const image, int exact) {
  int histo[4][256];
  const int kNumPredModes = 14;
  ...
  status = clEnqueueWriteBuffer(hardware.mQueue, residualpara.buffer_argb, CL_TRUE, (width + 1) * sizeof(uint32_t), frame_size, argb, 0, NULL, NULL);
  ...
  status = clEnqueueNDRangeKernel(hardware.mQueue, residualimage.mKernel, 2, 0,
  ...
  status = clEnqueueReadBuffer(hardware.mQueue, residualpara.buffer_residual, CL_TRUE, 0, residual_size, residual, 0, NULL, NULL);
  if(CL_SUCCESS != status) {
    fprintf(stderr, "%s %d %s\n", __func__, __LINE__, oclErrorCode(status));
    goto Error;
  }
...
void VP8LResidualImage(int width, int height, int bits, int low_effort,
                       uint32_t* const argb, uint32_t* const argb_scratch,
                       uint32_t* const image, int exact, int use_ocl) {
  // if (use_ocl) {
